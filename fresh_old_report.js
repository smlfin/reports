...
function generateReport() {
    ...
    let freshNet = 0;
    let oldNet = 0;
    const freshCustomers = new Set(); // For FRESH_CUSTOMER_REGULAR and FRESH_STAFF_CUSTOMER
    const oldCustomers = new Set();
    const freshStaffCustomers = new Set(); // For FRESH_STAFF_CUSTOMER
    ...
    const staffFreshCustomerMap = new Map(); 

    filteredDataForOverallAndCompany.forEach(row => {
        const rawCustomerType = String(row[freshOldColIndex]).trim().toUpperCase(); 
        let customerGroup; // Derived category

        if (rawCustomerType === 'FRESH CUSTOMER') {
            customerGroup = 'FRESH_CUSTOMER_REGULAR';
        } else if (rawCustomerType === 'FRESH CUSTOMER/MINIMUM AMT NIL') {
            customerGroup = 'FRESH_CUSTOMER_MIN_AMT_NIL';
        } else if (rawCustomerType === 'RESH STAFF/CUSTOMER') { // CORRECTED LINE FOR FRESH STAFF
            customerGroup = 'FRESH_STAFF_CUSTOMER'; // Internal category name
        } else {
            customerGroup = 'OLD_CUSTOMER'; // Covers 'OLD', blanks, and anything else
        }
        ...
        if (customerGroup === 'FRESH_CUSTOMER_REGULAR' || customerGroup === 'FRESH_STAFF_CUSTOMER') { // Both contribute to 'Fresh' totals
            freshInflow += currentInflow;
            freshNet += currentNet;
            if (customerName) freshCustomers.add(customerName);

            if (customerGroup === 'FRESH_STAFF_CUSTOMER') {
                if (customerName) freshStaffCustomers.add(customerName); // Separate count for fresh staff
            }
        ...
        } else if (customerGroup === 'OLD_CUSTOMER') {
            oldInflow += currentInflow;
            oldNet += currentNet;
            if (customerName) oldCustomers.add(customerName);
        }
        // FRESH_CUSTOMER_MIN_AMT_NIL is processed but not included in main freshCustomer count or totals per user request
    });
    ...
    if (staffFreshCustomerMap.size === 0) {
        staffFreshCustomerTableBody.innerHTML = '<tr><td colspan="4">No fresh customer data for staff with current filters.</td></tr>'; // Changed colspan to 4
    } else {
        // Get entries as [staffName, {customers: Set, totalInflow: number}], convert to array of objects
        ...
        // Sort by customerCount in descending order
        ...
        staffDataArray.forEach(staffEntry => {
            ...
            // Make Fresh Customers Brought (Count) clickable
            const countCell = row.insertCell();
            countCell.textContent = staffEntry.customerCount;
            countCell.classList.add('clickable-count'); // Add class for styling
            countCell.dataset.staffName = staffEntry.staffName; // Store staff name
            // Add event listener directly or via delegation if more efficient
            countCell.addEventListener('click', () => showCustomerDetailsDrilldown(staffEntry.staffName));

            // Add new cells for total and average inflow
            row.insertCell().textContent = formatIndianNumber(staffEntry.totalInflow); // Total Inflow
            const averageInflow = staffEntry.customerCount > 0 ? staffEntry.totalInflow / staffEntry.customerCount : 0;
            row.insertCell().textContent = formatIndianNumber(averageInflow); // Average Inflow
        });
    }
    ...
    filteredDataForMonthlyTrends.forEach(row => { // Use data filtered only by company/branch
        ...
        if (customerGroup === 'FRESH_CUSTOMER_REGULAR') {
        ...
        } else if (customerGroup === 'FRESH_STAFF_CUSTOMER') { // This now correctly matches 'RESH STAFF/CUSTOMER' from CSV
            monthlyData[yearMonth].Fresh.inflow += currentInflow;
            monthlyData[yearMonth].Fresh.net += currentNet;
            if (customerName) monthlyData[yearMonth].Fresh.customers.add(customerName);
            if (customerName) monthlyData[yearMonth].Fresh.staffCustomers.add(customerName); // Assuming customer name can also identify staff for staff count
        } else if (customerGroup === 'OLD_CUSTOMER') {
    ...
    monthlyFreshCustomerCountTableBody.innerHTML = ''; // Clear new table
    monthlyFreshStaffCustomerCountTableBody.innerHTML = ''; // Clear new table
...
function showCustomerDetailsDrilldown(staffName) {
    ...
    const staffData = staffFreshCustomerMap.get(staffName); // Use the globally populated map
...
totalFreshCustomersEl.addEventListener('click', showStaffPerformanceDrilldown); // Make Fresh Customers clickable

// --- NEW Event Listeners for the modals ---
closeStaffModalButton.addEventListener('click', closeStaffPerformanceModal); // Close button for staff modal
closeCustomerModalButton.addEventListener('click', closeCustomerDetailsModal); // Close button for customer details modal
